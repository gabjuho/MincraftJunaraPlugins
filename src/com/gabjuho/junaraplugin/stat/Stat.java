...
public class Stat {
    ...
    public void open(Player player) {
        ...
        meta1.setLore(Arrays.asList(ChatColor.GREEN + "현재 공격력 스텟: " + data.getConfig().get("player." + player.getUniqueId() + ".공격력포인트"), "스텟당 공격력이 0.1 증가합니다.", ChatColor.WHITE + "가진 스텟포인트: " + data.getConfig().get("player." + player.getUniqueId() + ".스텟포인트")));
        stat1.setItemMeta(meta1);

        stat2 = new ItemStack(Material.TOTEM_OF_UNDYING);
        meta2 = stat2.getItemMeta();
        meta2.setDisplayName("체력");
        meta2.setLore(Arrays.asList(ChatColor.GREEN + "현재 체력 스텟: "+ data.getConfig().get("player." + player.getUniqueId() + ".체력포인트"), "스텟당 체력이 0.5 증가합니다.", ChatColor.WHITE + "가진 스텟포인트: " + data.getConfig().get("player." + player.getUniqueId() + ".스텟포인트")));
        stat2.setItemMeta(meta2);

        stat3 = new ItemStack(Material.DIAMOND_CHESTPLATE);
        meta3 = stat3.getItemMeta();
        meta3.setDisplayName("방어력");
        meta3.setLore(Arrays.asList(ChatColor.GREEN + "현재 방어력 스텟: "+ data.getConfig().get("player." + player.getUniqueId() + ".방어력포인트"), "스텟당 방어력이 0.1 증가합니다.", ChatColor.WHITE + "가진 스텟포인트: " + data.getConfig().get("player." + player.getUniqueId() + ".스텟포인트")));
        stat3.setItemMeta(meta3);

        stat4 = new ItemStack(Material.FEATHER);
        meta4 = stat4.getItemMeta();
        meta4.setDisplayName("공격속도");
        meta4.setLore(Arrays.asList(ChatColor.GREEN + "현재 공격속도 스텟: "+ data.getConfig().get("player." + player.getUniqueId() + ".공격속도포인트"), "스텟당 공격속도가 0.1 증가합니다.", ChatColor.WHITE + "가진 스텟포인트: " + data.getConfig().get("player." + player.getUniqueId() + ".스텟포인트")));
        stat4.setItemMeta(meta4);

        stat5 = new ItemStack(Material.GOLDEN_BOOTS);
        meta5 = stat5.getItemMeta();
        meta5.setDisplayName("이동속도");
        meta5.setLore(Arrays.asList(ChatColor.GREEN + "현재 이동속도 스텟: "+ data.getConfig().get("player." + player.getUniqueId() + ".이동속도포인트"), "스텟당 이동속도가 0.1 증가합니다.", ChatColor.WHITE + "가진 스텟포인트: " + data.getConfig().get("player." + player.getUniqueId() + ".스텟포인트")));
